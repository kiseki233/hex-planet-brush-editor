...
class ProductionBuilder:
    def __init__(self, parent: tk.Misc, paths: ProjectPaths) -> None:
        ...
        self.window.title(t("千万格生产拓扑与地图构建器 v1.3.1"))
        ...
        self.status = tk.StringVar(value=t("尚未加载生产布局"))
    ...
    def _build_ui(self) -> None:
        ...
        self.build_button = ttk.Button(
            button_frame, text=t("生成/加载生产索引"), command=self.build_layout
        )
        self.build_button.grid(row=0, column=0, sticky="ew")
        self.create_button = ttk.Button(
            button_frame, text=t("创建完整空白 Pack 地图"), command=self.create_map, state=tk.DISABLED
        )
        self.create_button.grid(row=0, column=1, sticky="ew", padx=6)
        self.verify_button = ttk.Button(
            button_frame, text=t("逐区块验证地图"), command=self.verify_map, state=tk.DISABLED
        )
        ...
        ttk.Label(
            root,
            text=(
                t("该构建器不会展开一千万条 Python 几何对象。CellId、中心、邻居和六边形角点按需计算；"
    ...
    def build_layout(self) -> None:
        ...
        try:
            ...
            messagebox.showerror(t("错误"), t("频率和分块边长必须是整数"))
            return
        self._set_running(True)
        self.status.set(t("正在准备 frequency={frequency} 的生产索引……", frequency=frequency))
        threading.Thread(
        ...
        try:
            ...
            if cache_dir.exists():
                ...
                if layout.tile_side != tile_side:
                    raise ValueError(
                        t("现有缓存 tileSide={tile_side}，与输入的 {tile_side2} 不一致", tile_side=layout.tile_side, tile_side2=tile_side)
                    )
                source = t("读取缓存")
            else:
            ...
            if not validation.valid or not layout_validation.valid or not visibility_validation.valid:
                ...
                raise ValueError(t("；").join(issues[:8]))
            self.results.put((
                "layout",
    ...
    def create_map(self) -> None:
        ...
        if not name:
            messagebox.showerror(t("错误"), t("地图名称不能为空"))
            return
        self._set_running(True)
        self.status.set(t("正在创建完整空白 Pack 地图……"))
    ...
    def verify_map(self) -> None:
        ...
        name = self.map_name.get().strip()
        self._set_running(True)
        self.status.set(t("正在逐区块读取和验证地图……"))
        ...
        try:
            while True:
                ...
                if kind == "error":
                    ...
                    self._append(t("错误：{payload}", payload=payload))
                    messagebox.showerror(t("错误"), str(payload))
                elif kind == "layout":
                    ...
                    self._set_running(False)
                    self.status.set(t("生产索引已就绪"))
                    self.summary.set(
                        t("CellId：{cell_count:,}\n普通六边形：{hexagon_count:,}\n逻辑区块：{chunk_count:,}\n区块大小：{min}～{max}，平均 {value:.3f}\n展开的单格/三角记录：0 / 0", cell_count=layout.cell_count, hexagon_count=layout.topology.hexagon_count, chunk_count=layout.chunk_count, min=min(sizes), max=max(sizes), value=sum(sizes)/len(sizes))
                    )
                    self._append(
                        t("{source}完成，{visibility_source}：frequency={frequency}，区块={chunk_count:,}，可见性节点={node_count:,}，抽样检查={checked_chunks}，耗时={elapsed:.3f}s", source=source, visibility_source=visibility_source, frequency=layout.frequency, chunk_count=layout.chunk_count, node_count=visibility.node_count, checked_chunks=validation.checked_chunks, elapsed=elapsed)
                    )
                elif kind == "map":
                    session, action, elapsed = payload
                    self._set_running(False)
                    self.status.set(t("地图 {action}", action=action))
                    self._append(
                        t("地图 {name} {action}：index记录={len:,}，耗时={elapsed:.3f}s", name=session.name, action=action, len=len(session.index_records), elapsed=elapsed)
                    )
                elif kind == "verify":
                    report, elapsed = payload
                    self._set_running(False)
                    self.status.set(t("地图验证通过") if report.valid else t("地图验证失败"))
                    self._append(
                        t("逐区块验证：valid={valid}，checked={checked_chunks:,}，failed={len}，耗时={elapsed:.3f}s", valid=report.valid, checked_chunks=report.checked_chunks, len=len(report.failed_chunks), elapsed=elapsed)
                    )
