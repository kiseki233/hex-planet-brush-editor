...
class EditorApp:
    def __init__(self, root: tk.Tk, paths: ProjectPaths) -> None:
        ...
        self.status_text = tk.StringVar(value=t("准备就绪"))
        self.side = 24
    ...
    def _configure_root(self) -> None:
        self.root.title(t("六边形星球地图笔刷编辑器 v1.3.1"))
    ...
    def _build_brush_panel(self, parent: ttk.Frame) -> None:
        ...
        ttk.Button(frame, text=t("刷新笔刷库"), command=self.refresh_brushes).grid(row=0, column=0, sticky="ew", pady=(0, 6))
    ...
    def _build_canvas_panel(self, parent: ttk.Frame) -> None:
        frame = ttk.LabelFrame(parent, text=t("16×16 局部六边形编辑原型"), padding=6)
    ...
    def _build_control_panel(self, parent: ttk.Frame) -> None:
        frame = ttk.LabelFrame(parent, text=t("地图与工具"), padding=10)
        frame.grid(row=0, column=2, sticky="ns", padx=(8, 0))
        frame.columnconfigure(0, weight=1)

        ttk.Label(frame, text=t("地图名称")).grid(row=0, column=0, sticky="w")
        ...
        ttk.Button(button_row, text=t("新建"), command=self.new_map).pack(side=tk.LEFT, expand=True, fill=tk.X)
        ttk.Button(button_row, text=t("保存"), command=self.save_map).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(5, 0))

        ttk.Separator(frame).grid(row=3, column=0, sticky="ew", pady=10)
        ttk.Label(frame, text=t("打开已有地图")).grid(row=4, column=0, sticky="w")
        ...
        ttk.Button(frame, text=t("打开"), command=self.open_map).grid(row=6, column=0, sticky="ew")

        ttk.Separator(frame).grid(row=7, column=0, sticky="ew", pady=10)
        ttk.Label(frame, text=t("编辑工具")).grid(row=8, column=0, sticky="w")
        ttk.Radiobutton(frame, text=t("放置笔刷"), variable=self.selected_tool, value="paint").grid(row=9, column=0, sticky="w")
        ttk.Radiobutton(frame, text=t("清除格子"), variable=self.selected_tool, value="erase").grid(row=10, column=0, sticky="w")

        ttk.Label(frame, text=t("旋转方向")).grid(row=11, column=0, sticky="w", pady=(10, 3))
        ...
        ttk.Label(frame, text=t("当前笔刷")).grid(row=13, column=0, sticky="w", pady=(12, 3))
        ...
        ttk.Button(frame, text=t("打开球面拓扑检查器"), command=self.open_topology_viewer).grid(
            row=17, column=0, sticky="ew"
        )

        ttk.Button(frame, text=t("打开分块与 Pack 检查器"), command=self.open_chunk_storage_viewer).grid(
            row=18, column=0, sticky="ew", pady=(6, 0)
        )

        ttk.Button(frame, text=t("打开球面地图编辑器"), command=self.open_sphere_editor).grid(
            row=19, column=0, sticky="ew", pady=(6, 0)
        )

        ttk.Button(frame, text=t("打开千万格生产构建器"), command=self.open_production_builder).grid(
            row=20, column=0, sticky="ew", pady=(6, 0)
        )

        ttk.Button(frame, text=t("打开千万格生产 GPU 编辑器"), command=self.open_production_editor).grid(
            row=21, column=0, sticky="ew", pady=(6, 0)
        )

        ttk.Button(frame, text=t("运行最终验收与 Windows 诊断"), command=self.open_acceptance_viewer).grid(
        ...
        ttk.Label(
            frame,
            text=(
                t("当前版本已实装：笔刷与局部编辑、稳定球面拓扑、连通分块、层级区块可见性索引、"
    ...
    def refresh_brushes(self, show_dialog: bool = True) -> None:
        ...
        if show_dialog:
            messagebox.showinfo(t("笔刷库"), self.status_text.get())
        ...
        if self.scan_result is not None:
            ...
            for record in self.scan_result.records:
                if record.state == "missing":
                    lines.append(t("缺失：{relative_path}\n  UID 保留，地图引用未清除", relative_path=record.relative_path))
        if not lines:
    ...
    def _on_canvas_click(self, event: tk.Event) -> None:
        ...
        if self.selected_tool.get() == "erase":
            ...
            if self.selected_brush_uid is None:
                self.status_text.set(t("请先选择一张有效笔刷"))
                return
            record = self.records_by_uid.get(self.selected_brush_uid)
            if record is None or record.state != "active":
                self.status_text.set(t("当前笔刷文件缺失，不能放置"))
                return
            self.current_map.set_cell(index, record.uid, record.relative_path, self.rotation.get())
        self._redraw_map()
        self.status_text.set(t("已修改格子 {index}，地图尚未保存", index=index))
    ...
    def new_map(self) -> None:
        ...
        try:
            ...
            messagebox.showerror(t("地图名称无效"), str(exc))
            return
        self.map_name.set(self.current_map.name)
        self._redraw_map()
        self.status_text.set(t("已新建局部地图：{name}", name=self.current_map.name))
    ...
    def save_map(self) -> None:
        try:
            ...
            messagebox.showerror(t("保存失败"), str(exc))
            return
        self.map_name.set(self.current_map.name)
        self._refresh_map_choices()
        self.status_text.set(t("地图已保存：{map_dir}", map_dir=map_dir))

    def open_map(self) -> None:
        name = self.map_choice.get().strip()
        if not name:
            messagebox.showinfo(t("打开地图"), t("没有可打开的地图"))
            return
        if not self._confirm_discard_changes():
            return
        try:
            ...
            messagebox.showerror(t("打开失败"), str(exc))
            return
        self.map_name.set(self.current_map.name)
        self._redraw_map()
        self.status_text.set(t("已打开地图：{name}", name=name))
    ...
    def _confirm_discard_changes(self) -> bool:
        if not self.current_map.dirty:
            return True
        return messagebox.askyesno(t("尚未保存"), t("当前地图有未保存修改，确定继续吗？"))
